...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const title = `${video.actresses.join(", ")} — ${video.sceneTitle} | ${SITE.name}`;
  return {
    title,
    ...
    "@graph": [
      {
        ...
        itemListElement: [
          ...
          { "@type": "ListItem", position: 3, name: video.sceneTitle, item: pageUrl },
        ],
      },
      {
        "@type": "VideoObject",
      ...
      <main className="site-container content-layout detail-layout">
        <article className="detail-content">
          ...
          <header className="detail-heading">
            ...
            <p>{video.year} · {video.duration} · {video.rating}% rating</p>
          </header>
          ...
          <div className="detail-data" aria-label="Scene information">
            <dl>
              <div><dt>Title</dt><dd>{video.sceneTitle}</dd></div>
              <div><dt>Year</dt><dd><Link href={`/year/${video.year}`}>{video.year}</Link></dd></div>
              <div><dt>Type</dt><dd><Link href={typePath}>{video.type}</Link></dd></div>
              <div><dt>Duration</dt><dd>{video.duration}</dd></div>
            </dl>
