  ...
  return (
    <div className={`filter-shell${open ? " open" : ""}`}>
      ...
      <form id="catalog-filter-form" className="catalog-filters" action={basePath} method="get" aria-busy={pending} onSubmit={applyFilters}>
        ...
        {!hideYear && <label htmlFor="filter-year"><span>Year</span><SelectMenu key={`year-${values.year ?? "all"}`} id="filter-year" name="year" ariaLabel="Filter by release year" defaultValue={values.year ?? ""} options={yearOptions} /></label>}
        ...
        <label htmlFor="filter-rating"><span>Rating</span><SelectMenu key={`rating-${values.minRating ?? "all"}`} id="filter-rating" name="rating" ariaLabel="Filter by minimum rating" defaultValue={values.minRating ?? ""} options={ratingOptions} /></label>
